"""
Correct xBD raster metadata (CRS and Geotransform) by creating lightweight .vrt files.

These VRTs point to the original .tif files but contain the correct UTM projections
and bounds based on the metadata files.

Warning:
    If the original .tif files are moved or deleted, the VRTs will break since they
...
def create_all_aligned_vrt_files(original_folder: Path, overwrite: bool = False, num_workers: int = 8) -> None:
    ...
    """
    assert original_folder.exists(), f"Original xBD folder does not exist: {original_folder}"
    ...
    # Global checks to see if all VRT files already exist.
    if not overwrite:
        all_exists = all([(output_folder / f"{uid}_{period}_disaster.vrt").exists() for uid in df_meta.index for period in ["pre", "post"]])
        if all_exists:
            print(f"All VRT files already exist in {output_folder}. Skipping VRT creation.")
            return
